...
v4/v5/v6/v8 融合；最新合成候选数据版本为 `standard-v9`：

```bash
python scripts/generate_grouped_data.py -o data/pieces_grouped_v9 -n 3000
python scripts/audit_dataset.py data/pieces_grouped_v9 --against-manifest evaluation/standard_manifest.csv
python scripts/train_v2.py -d data/pieces_grouped_v9 -o data/models/checkpoint_standard_v9.pth
```
...
`standard-v9` 将 screen 与 sparse_endgame 占比进一步提高，并对稀疏残局棋子位置做边缘/
九宫偏置。`checkpoint_standard_v9.pth` 单模型标准集为 5393/5400、53/60，真实集为
1321/1620、5/18；小剂量融合未超过 v8 主线，因此不晋级，仅作为 screen 残局数据实验记录。
...
视觉占位补全、候选面积先验、士/象静态点概率先验与稀疏残局身份先验后，
在固定清单上达到：标准评测 5400/5400 格正确、60/60 盘完全正确；真实照片回归集
1546/1620 格正确（95.43%）、整盘 12/18。screen 子集提升到 599/630、整盘 3/7，
wood 子集达到 615/630、整盘 6/7，initial 子集保持 720/720、整盘 8/8；剩余错误主要转向
