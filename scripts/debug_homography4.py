...
from src.preprocess.board_detector import (
    _cluster_rhos,
    _hough_intersection,
)
...
for canny_low, canny_high, hough_thresh_factor in [
    (40, 120, 0.15),
    (40, 120, 0.12),
    (50, 150, 0.10),
    (30, 90, 0.08),
    ...
    print(
        f"  canny={canny_low},{canny_high} thresh={threshold} "
        f"h_lines={len(h_rhos)} v_lines={len(v_rhos)}"
    )
    ...
    if len(h_clusters) >= 3 and len(v_clusters) >= 3:
        ...
        for rho_h in h_clusters:
            ...
            for rho_v in v_clusters:
                ...
                print(
                    f"    rho_h={rho_h:.0f} rho_v={rho_v:.0f} -> "
                    f"col={col:.1f} row={row:.1f} src=({sx:.0f},{sy:.0f})"
                )
