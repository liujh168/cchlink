...
pipeline = Pipeline(
    model_path=MODEL_PATH, device="cuda" if __import__("torch").cuda.is_available() else "cpu"
)
...
for i in range(num_tests):
    ...
    cv2.imwrite(
        os.path.join(debug_dir, f"test_{i:03d}_input.png"), cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    )
    ...
    if corners is not None:
        ...
        print("  saved cell images to debug dir")
    else:
        print("  corners FAILED, trying homography...")
