...
print(f"\nTop edge region (0-{gray.shape[0]//4}): peaks at {np.argmax(h_profile[:gray.shape[0]//4])}")
print(f"Bot edge region ({3*gray.shape[0]//4}-{gray.shape[0]}): peaks at {3*gray.shape[0]//4 + np.argmax(h_profile[3*gray.shape[0]//4:])}")
print(f"Left edge region (0-{gray.shape[1]//4}): peaks at {np.argmax(v_profile[:gray.shape[1]//4])}")
print(f"Right edge region ({3*gray.shape[1]//4}-{gray.shape[1]}): peaks at {3*gray.shape[1]//4 + np.argmax(v_profile[3*gray.shape[1]//4:])}")
