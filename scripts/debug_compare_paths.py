...
for test_id in ['004', '007', '016']:
    ...
    if corners is not None:
        ...
        if cells:
            print(f'  Corner cell sizes range: {min(c.shape[0] for c in cells)}x{min(c.shape[1] for c in cells)} to {max(c.shape[0] for c in cells)}x{max(c.shape[1] for c in cells)}')
            print(f'  Corner cell means range: {min(float(c.mean()) for c in cells):.1f} to {max(float(c.mean()) for c in cells):.1f}')
    ...
    if H is not None:
        ...
        for r in range(10):
            for c in range(9):
                ...
                cv2.imwrite(rf'i:\cchlink\debug\cells\homography\test_{test_id}\cell_{r}_{c}.png', cell)
                idx += 1
    print()